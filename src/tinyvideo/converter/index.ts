...
const processFrames = () => {
  ...
  for (const frame of frames) {
    imagemagick.convert([path.join(outdir, frame), '-monochrome', path.join(outdir, frame)], err => {
      if (err) {
        throw err;
      }
...
const processFrame = (frame: Buffer) => {
  const rawColourTableStart = new Uint8Array(BMP_HEADER_COLOUR_TABLE_OFFSET_LENGTH_BYTES);
  const copied = frame.copy(rawColourTableStart, 0, BMP_HEADER_COLOUR_TABLE_OFFSET_START, BMP_HEADER_COLOUR_TABLE_OFFSET_START + BMP_HEADER_COLOUR_TABLE_OFFSET_LENGTH_BYTES);

  if (copied !== BMP_HEADER_COLOUR_TABLE_OFFSET_LENGTH_BYTES) {
    throw new Error(`Unexpected read byte count for Header.DataOffset! Got ${copied} but wanted ${BMP_HEADER_COLOUR_TABLE_OFFSET_LENGTH_BYTES}`);
  }

  const colourTableStart = rawColourTableStart.reduce((total, x) => total + x, 0);
